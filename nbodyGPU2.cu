...
// Both the force calculation and the movment are done on the GPU																																												
// nvcc nbodyGPU2.cu -o GPU2 -lglut -lm -lGLU -lGL
...
{
	vel[threadIdx.x].x += ((force[threadIdx.x].x-DAMP*vel[threadIdx.x].x)/pos[threadIdx.x].w)*DT;
	vel[threadIdx.x].y += ((force[threadIdx.x].y-DAMP*vel[threadIdx.x].y)/pos[threadIdx.x].w)*DT;
	vel[threadIdx.x].z += ((force[threadIdx.x].z-DAMP*vel[threadIdx.x].z)/pos[threadIdx.x].w)*DT;
	
	pos[threadIdx.x].x += vel[threadIdx.x].x*DT;
	pos[threadIdx.x].y += vel[threadIdx.x].y*DT;
	pos[threadIdx.x].z += vel[threadIdx.x].z*DT;
}
