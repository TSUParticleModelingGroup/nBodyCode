// Straight forward GPU code made to run on multiple blocks. No shared memory or any optimization.
// Initial conditions set in a cube.																																												
// nvcc nbodyGPU3.cu -o GPU3 -lglut -lm -lGLU -lGL
...
__global__ void getForces(float4 *pos, float3 *vel, float3 * force)
{
    ...
    if(id <N)
    {
        ...
        for(int i=0; i<N; i++)	
        {
		    if(i != id) 
		    {
				    force_mag = getBodyBodyForce(pos[id], pos[i]);
				    force[id].x += force_mag.x;
				    force[id].y += force_mag.y;
				    force[id].z += force_mag.z;
		    }
	    }
	}
}
