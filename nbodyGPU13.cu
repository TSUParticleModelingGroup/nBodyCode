// Same as GPU6 but fixing the number of blocks																																							
// nvcc nbodyGPU13.cu -o GPU13 -lglut -lm -lGLU -lGL
...
void set_initail_conditions()
{
	...
	grid.x = 2*prop.multiProcessorCount;
	grid.y = 1;
	grid.z = 1;
...
__global__ void getForces(float4 *pos, float3 *vel, float3 * force)
{
    ...
    m = N/(blockDim.x*gridDim.x); //N must divide evenly by BLOCK*GRID
	    ...
	    for(k = 0; k < m; k++)
	    {
	        id = threadIdx.x + blockDim.x*blockIdx.x + k*blockDim.x*gridDim.x;
    	    ...
    	    {
    			shPos[threadIdx.x] = pos[threadIdx.x + blockDim.x*j + k*blockDim.x*gridDim.x];
    			__syncthreads();
   
			    #pragma unroll 32
        	    for(int i=0; i < blockDim.x; i++)	
        	    {
        		    ii = i + blockDim.x*j + k*blockDim.x*gridDim.x;
		    	    if(ii != id ) 
		    	    {
